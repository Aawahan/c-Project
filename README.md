...
int is_valid_expression() {
    int len = strlen(expr), paren = 0, hasDigit = 0;
    char temp[200];
    int j = 0;

    for (int i = 0; i < len; ++i)
        if (expr[i] != ' ') temp[j++] = expr[i];
    temp[j] = '\0';
    strcpy(expr, temp);

    len = strlen(expr);
    if (len == 0) return 0;
    if (strchr("*/^)", expr[0])) return 0;
    ...
    if (isalpha((unsigned char)current())) {
        if (match_function("sqrt")) {
            if (current() == '(') {
                ...
                if (current() == ')') next();
                value = sqrtf(v);
            } else value = sqrtf(factor());
        }
        else if (match_function("log")) {
            if (current() == '(') { next(); float v = expression(); if (current()==')') next(); value = log10f(v); }
            else value = log10f(factor());
        }
        else if (match_function("ln")) {
            if (current() == '(') { next(); float v = expression(); if (current()==')') next(); value = logf(v); }
            else value = logf(factor());
        }
        else {
            while (isalpha((unsigned char)current())) next();
            value = 0.0f;
        }
    }
    else if (current() == '(') {
